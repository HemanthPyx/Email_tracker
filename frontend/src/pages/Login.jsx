/**
 * Login page — Google OAuth sign-in.
 */
import { useState } from 'react';
import { authAPI } from '../services/api';
import { HiOutlineMail, HiOutlineShieldCheck, HiOutlineSearch, HiOutlineLightningBolt } from 'react-icons/hi';

const features = [
  {
    icon: HiOutlineLightningBolt,
    title: 'Fully Automated',
    desc: 'Scans your Gmail automatically — zero manual entry needed',
  },
  {
    icon: HiOutlineSearch,
    title: 'Instant Search',
    desc: 'Find which email you used for any site in seconds',
  },
  {
    icon: HiOutlineShieldCheck,
    title: 'Secure & Private',
    desc: 'Read-only access, data stays yours, OAuth2 secured',
  },
];

export default function Login() {
  const [loading, setLoading] = useState(false);

  const handleGoogleLogin = async () => {
    setLoading(true);
    try {
      const redirectUri = `${window.location.origin}/auth/callback`;
      const res = await authAPI.getGoogleAuthUrl(redirectUri);
      if (res.data.code_verifier) {
        sessionStorage.setItem('code_verifier', res.data.code_verifier);
      }
      window.location.href = res.data.auth_url;
    } catch (err) {
      console.error('Failed to get auth URL:', err);
      setLoading(false);
    }
  };

  return (
    <div className="min-h-screen bg-dark-bg flex">
      {/* Left — Branding */}
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-center px-16 relative overflow-hidden">
        {/* Background decoration */}
        <div className="absolute top-20 -left-20 w-72 h-72 bg-brand-blue/5 rounded-full blur-3xl" />
        <div className="absolute bottom-20 right-10 w-60 h-60 bg-brand-green/5 rounded-full blur-3xl" />

        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-12 h-12 rounded-xl bg-brand-blue/15 flex items-center justify-center">
              <HiOutlineMail className="w-7 h-7 text-brand-blue" />
            </div>
            <h1 className="text-3xl font-bold text-text-primary tracking-tight">
              Mail<span className="text-brand-blue">Tracker</span>
            </h1>
          </div>

          <p className="text-xl text-text-secondary mb-12 leading-relaxed max-w-lg">
            Never forget which email you used to sign up for a website. 
            We scan your inboxes and build a searchable map of all your registrations.
          </p>

          <div className="space-y-6">
            {features.map((f, i) => (
              <div key={i} className="flex items-start gap-4 animate-slide-up" style={{ animationDelay: `${i * 100}ms` }}>
                <div className="w-10 h-10 rounded-lg bg-dark-surface border border-dark-border flex items-center justify-center flex-shrink-0">
                  <f.icon className="w-5 h-5 text-brand-blue" />
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-text-primary mb-0.5">{f.title}</h3>
                  <p className="text-sm text-text-secondary">{f.desc}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* Right — Login */}
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        <div className="w-full max-w-sm">
          {/* Mobile logo */}
          <div className="lg:hidden flex items-center gap-3 mb-8 justify-center">
            <div className="w-10 h-10 rounded-xl bg-brand-blue/15 flex items-center justify-center">
              <HiOutlineMail className="w-6 h-6 text-brand-blue" />
            </div>
            <h1 className="text-2xl font-bold text-text-primary tracking-tight">
              Mail<span className="text-brand-blue">Tracker</span>
            </h1>
          </div>

          <div className="card p-8">
            <h2 className="text-xl font-bold text-text-primary mb-2 text-center">
              Welcome back
            </h2>
            <p className="text-sm text-text-secondary mb-8 text-center">
              Sign in with your Google account to get started
            </p>

            <button
              onClick={handleGoogleLogin}
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-lg
                         bg-white text-gray-800 font-medium text-sm
                         hover:bg-gray-50 active:scale-[0.98]
                         transition-all duration-200
                         disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <div className="w-5 h-5 border-2 border-gray-300 border-t-gray-600 rounded-full animate-spin" />
              ) : (
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/>
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
              )}
              {loading ? 'Connecting...' : 'Continue with Google'}
            </button>

            <p className="text-xs text-text-muted text-center mt-6 leading-relaxed">
              We request read-only access to detect registration emails. 
              Your data stays private and secure.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}
